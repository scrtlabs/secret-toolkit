...
impl<'a, K: Serialize + KeyDeserialize, T: Serialize + DeserializeOwned, Ser: Serde>
    Keymap<'a, K, T, Ser>
{
    /// constructor
    pub const fn new(namespace: &'a [u8]) -> Self {
    ...
    Keymap<'a, K, T, Ser, WithIter>
{
    ...
    fn get_from_deserialized_key(&self, storage: &dyn Storage, key: &K::Output) -> StdResult<InternalItem<T, Ser>> {
    ...
    Ser: Serde,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // turn key into pair
        let pair = match key {
            Ok(k) => match self.keymap.get_from_deserialized_key(self.storage, &k) {
...
mod tests {
    ...
    fn test_keymap_iter_keys() -> StdResult<()> {
        ...
        keymap.insert(&mut storage, key1.clone(), &foo1)?;
        keymap.insert(&mut storage, key2.clone(), &foo2)?;
    ...
    fn test_keymap_length_with_page_size(page_size: u32) -> StdResult<()> {
        ...
        keymap.insert(&mut storage, key2.clone(), &foo1)?;
        assert_eq!(keymap.get_len(&storage)?, 1);
        assert!(keymap.length.lock().unwrap().eq(&Some(1)));
